	...
	success : function(data) {
		...
		$.each(data.staffquarter, function(i, item) {
			$.each(item, function(key, value) {
					...
					}else if (value == true && key == 'Has_Separate_Meter') {
						$('input:radio[name='+key+"_"+i+']')[0].checked = true;
					} else if(value== false && key == 'Has_Separate_Meter') {
						$('input:radio[name='+key+"_"+i+']')[1].checked = true;
					}
...
$('#staffQuartersSaveDiv').on('click',function(){
	...
	$('#staff >div').each(function(){
		...
		divJson['hasSeperateMeter'] = $($this).find('>div >div >div.panel-collapse >div >li').find('input[type="radio"]').is(':checked') ? 'Y' : 'N';
